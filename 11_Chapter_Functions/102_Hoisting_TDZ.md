...
Important: Only declarations are hoisted, NOT initializations or assignments. The variable exists in memory from the start of the scope, but its value is only assigne
d when execution reaches that line.
...
| Function Expression with `var`   | Variable only | ❌ TypeError                  |
| Function Expression with `let`   | TDZ           | ❌ ReferenceError             |
| Function Expression with `const` | TDZ           | ❌ ReferenceError             |
